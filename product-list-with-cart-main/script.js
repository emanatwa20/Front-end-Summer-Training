document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeCart = () => {
        document.querySelectorAll('.products-card').forEach(card => {
            ...
            addToCartButton.addEventListener('click', () => handleAddToCart(productName, productPrice, addToCartButton, counterElement, productImage));
            card.querySelector('.icon-increment').addEventListener('click', () => handleIncrement(productName, productPrice, counterElement));
            card.querySelector('.icon-decrement').addEventListener('click', () => handleDecrement(productName, counterElement, addToCartButton, productImage));
        });
    };

    const handleAddToCart = (productName, productPrice, addToCartButton, counterElement, productImage) => {
    ...
    const handleIncrement = (productName, productPrice, counterElement) => {
        cart[productName] = cart[productName] || { price: productPrice, quantity: 0 };
        cart[productName].quantity += 1;
        counterElement.querySelector('span').textContent = cart[productName].quantity;
        updateCartDisplay();
    ...
    const handleDecrement = (productName, counterElement, addToCartButton, productImage) => {
    ...
    const updateCartDisplay = () => {
        ...
        for (const [productName, product] of Object.entries(cart)) {
            ...
            cartItem.innerHTML = `
                ...
                <svg class="icon-remove" xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10">
            ...
            cartItem.querySelector('.icon-remove').addEventListener('click', () => removeCartItem(productName, cartItem));
        }
    ...
    const removeCartItem = (productName, cartItem) => {
        delete cart[productName];
        document.querySelectorAll('.products-card').forEach(card => {
        ...
        cartItem.remove(); // Remove the item from cart display
        updateCartDisplay();
    };
    ...
    const resetCart = () => {
        ...
        document.querySelectorAll('.counter').forEach(counter => {
            ...
            counter.querySelector('span').textContent = '0'; // Reset counter to 0
        });
